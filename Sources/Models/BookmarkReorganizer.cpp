    ...
    try {
        ...
        for (const auto& comp : pathComponents) {
            ...
            if (!directory_exists(fullPath)) {
                ...
                // Change to the current path before creating directory
                std::wstring originalDir = get_current_working_directory();
                if (!change_working_directory(currentPath)) {
                    DEBUG_LOG("Failed to change to directory: " << narrow(currentPath));
                    DEBUG_LOG_FUNC_EXIT();
                    return false;
                }
                
                // Create directory
                make_directory(safeName);
                
                // Change back to original directory
                change_working_directory(originalDir);
                
                // Verify creation and log result
                bool creationSuccess = directory_exists(fullPath);
