...
#include "Models/OllamaService.h"
#include "Models/BookmarkReorganizer.h"
#include "Utils/Errors.h"
#include "Utils/Miscellaneous.h"
#include "Views/AddDirectory.h"
#include "Views/AddLink.h"
#include "Views/AutoFolderStatus.h"
...
#include "Views/DeleteDirectory.h"
#include "Views/DeleteLink.h"
#include "Views/EditDirectory.h"
#include "Views/EditLink.h"
                    ...
                    if (get_query_parameter(0) ==
                        ...
                        if ((username = (TCHAR *)wcsrchr(query, '='))) {
                            ...
                            if (get_query_parameter(0) == CMD_LOG_IN) {
                                ...
                                // TODO: Use special function to format wstrings.
                                throw two_msg_exception(err_invalid_login, std::wstring(L"[") +
                                                                               username + L"/" +
                                                                               password + L"]");
                            } else
        ...
        if (!wcscmp(link, ok)) {
            if (get_query_parameter(0) == CMD_ADD) {  // Now after = url of the new link.
                ...
                if (url) {
                    TCHAR *and_;
                    url++;
                    // The & symbol must be removed from the new url.
                    if ((and_ = (TCHAR *)wcsrchr(url, '&')))
                        *and_ = 0;
...
void do_auto_folder()
{
    ...
    try {
        ...
        DEBUG_LOG("Bookmark reorganizer created successfully");
...
void get_key()
{
    ...
    if (CMD_KEY == get_query_parameter(0))  // Removing the key from the request.
    {
        ...
        if (!std::filesystem::exists(key_file_path))
            file_doesnt_exist(key_file_path);
