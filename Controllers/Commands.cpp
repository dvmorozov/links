...
void do_edit_conf()
{
    ...
    if (link)
    {
        ...
        if (!fatal_error)
        {
            ...
            {
                wprintf(htm_edit, full_script_name, name.c_str(), query, url.c_str(), key, link, key);
            }
        }
    }
}
...
void do_log_in_conf()
{
    wprintf(htm_log_in_conf, full_script_name);
}
//-------------------------------------------------------------------------------------------------
...
void do_del_conf()
{
    ...
    if (link)
    {
        ...
        if (!fatal_error)
        {
            ...
            name = link;
#endif
            wprintf(htm_del_conf, full_script_name, name.c_str(), url.c_str(), query, link, key, query, link, key);
        }
        else invalid_query();
    }
    else invalid_query();
...
void do_edit_folder_conf()
{
    ...
    {
        link++;
        wprintf(htm_edit_folder, full_script_name, link, query, key, link, key);
    }
}
//-------------------------------------------------------------------------------------------------
...
void do_add_folder_conf()
{
    ...
    if (!fatal_error)
    {
        wprintf(htm_add_folder, full_script_name, query, key, key);
    }
}
//-------------------------------------------------------------------------------------------------
...
void do_del_folder_conf()
{
    ...
    {
        folder++;
        wprintf(htm_folder_del_conf, full_script_name, folder, query, key, query, folder, key);
    }
    else invalid_query();
}
//-------------------------------------------------------------------------------------------------
...
#endif
{
    ...
    //  https://action.mindjet.com/task/14817423
    prepare_query_buffer(wcslen(encodedQuery));
