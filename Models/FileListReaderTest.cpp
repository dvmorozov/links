...
namespace Bookmarks
{
    ...
    FileListReader *GetFileReader()
    {
        ...
        CopyFile(
            ...
            (Bookmarks::RegConfig::GetValue(_T("TestFolder")) + _T("\\Тест\\57564efb.url")).c_str(),
            FALSE
        );
        CopyFile(
            (Bookmarks::RegConfig::GetValue(_T("TestFolder")) + _T("\\14732139\\57584c98.url")).c_str(),
            (Bookmarks::RegConfig::GetValue(_T("TestFolder")) + _T("\\Тест\\57584c98.url")).c_str(),
            FALSE
        );
        CopyFile(
            (Bookmarks::RegConfig::GetValue(_T("TestFolder")) + _T("\\14732139\\57584dec.url")).c_str(),
            (Bookmarks::RegConfig::GetValue(_T("TestFolder")) + _T("\\Тест\\57584dec.url")).c_str(),
            FALSE
        );
        return new FileListReaderTest(Bookmarks::RegConfig::GetValue(_T("TestFolder")) + _T("\\Тест\\test.txt"));
    }
}
