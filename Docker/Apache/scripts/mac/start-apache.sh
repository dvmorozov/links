#!/bin/bash
# macOS startup script for links-legacy-apache container
...
echo "🍎 Starting Links Legacy Apache on macOS..."
...
# Generate .env from macOS template
echo "📝 Generating .env file for macOS (user: $USER)..."
substitute_env_vars "$PROJECT_DIR/.env.mac" "$PROJECT_DIR/.env"
...
if [ ${#MISSING_PATHS[@]} -gt 0 ]; then
    ...
    echo "   2. Adjust paths in .env.mac if needed"
    echo ""
...
case "$ACTION" in
    ...
    logs)
        ...
        ;;
    env-only)
        echo "📝 Generated .env file for macOS (environment setup only)"
